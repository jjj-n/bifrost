...
func TestEnableClient_StillPendingVerification_KeepsThatState(t *testing.T) {
	for _, authType := range []schemas.MCPAuthType{schemas.MCPAuthTypePerUserOauth, schemas.MCPAuthTypeOauth} {
		t.Run(string(authType), func(t *testing.T) {
			...
			config := newPendingClientConfig("client-enable-pending", authType)
