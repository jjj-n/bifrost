...
func (m *ToolsManager) executeToolInternal(
	...
	if callErr != nil {
		...
		if isAuthFailureErrorText(callErr.Error()) || isDeadSessionErrorText(callErr.Error()) {
			if retryResponse, recovered := m.attemptCallFailureRecovery(ctx, toolName, callRequest, executionConfig, toolExecutionTimeout); recovered {
...
// (isDeadSessionErrorText). Both land here because the remedy is identical;
