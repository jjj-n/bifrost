...
- **Automatic recovery:** a failed check reconnects the client in the background, so a session the server expired, a dropped SSE stream, or a `stdio` server whose process exited is replaced without anyone intervening. The reconnect keeps the current connection serving until the replacement is ready
...
A per-client `0` or unset value inherits the global; an unset global means 10 minutes. See [State reference](./connections) for what each state means and how recovery works per connection mode.

Recovery is automatic, but you can also force a reconnect at any time using the API or Go SDK:
