...
Unlike reconnect, this applies to **every** client type. A sticky client is re-listed over its live connection; a per-call client (and every per-user auth type) goes through a one-shot connect, `tools/list`, disconnect cycle using its admin discovery credential — which is the only on-demand refresh path those clients have.
