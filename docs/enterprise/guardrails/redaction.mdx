...
## Tool-Call Arguments in LLM Guardrails

Guardrail providers in rules that protect LLM traffic inspect assistant tool calls by default. Input rules cover calls in the selected conversation history; output rules cover calls generated by the model. This includes Chat function arguments and Responses function arguments or custom-tool input. Integrations receive either structured tool calls or an equivalent canonical text record, depending on the provider.

Custom Regex, Secrets Detection, Microsoft Presidio, and Azure AI Language PII can redact argument values with the same strategy and mode as other text. Tool names, IDs, and definitions are not redaction targets. Runtime redaction can change what a bash, grep, or other command does; Bifrost returns the redacted arguments without repairing or restoring the command. `logs_only` preserves the call sent to the client and records redaction mappings for logs and traces.
...
Bedrock and Model Armor can apply provider-managed transformations to tool-call arguments when Bifrost can safely rewrite the client representation. MCP-targeted rules continue to evaluate actual MCP execution arguments and results through their existing adapters. Lakera retains its existing argument-mapping restrictions.
