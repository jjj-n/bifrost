...
<Tab title="Singulr AI">

Calls Singulr AI's Bifrost guardrail endpoint. The Singulr policy decides whether content is allowed, blocked, or redacted. For a `redact` decision, Bifrost validates Singulr's spans and applies fixed runtime replacement.
...
`base_url` is optional and defaults to `https://dev-app6.ai-platform.singulr.ai`. Bifrost appends `/api/v1/ai-gateway/bifrost`; set `base_url` only when Singulr provides a different API root. See [Singulr AI](/integrations/guardrails/singulr-ai) for setup, streaming behavior, and supported content.
...
| `provider_name` | Yes | Backend: `"regex"`, `"secrets"`, `"prompt-guardrail"`, `"presidio"`, `"azure-pii"`, `"bedrock"`, `"azure"`, `"model-armor"`, `"crowdstrike-aidr"`, `"patronus-ai"`, `"grayswan"`, `"lakera"`, `"repelloai-argus"`, `"singulr-ai"` |
...
| Field | Required | env.* supported | Notes |
|-------|----------|-----------------|-------|
| `api_key` | Yes | Yes | Singulr API key |
| `application_id` | Yes | **Plain only** | Complete Application ID supplied by Singulr |
| `guardrail_id` | Yes | **Plain only** | Singulr guardrail configuration ID |
| `base_url` | No | Yes | Singulr API root. Defaults to `https://dev-app6.ai-platform.singulr.ai` |
| `timeout` | No | **Plain only** | Provider execution timeout in seconds |
